...
with open(args.filename, mode='r') as data_file:
    ...
    for line in data_file:
        ...
        if 'name' in line:
            ...
            if heade_len != len(record_accumulator):
                if args.verbosity > 0:
                    print(f"Warning: header len({heade_len}) != record len({len(record_accumulator)})")
